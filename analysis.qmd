...
Our team decided to work with the datasets that could give us more information to help us understand the complex dynamics at play within the city's socio-economic and racial landscape.The datasets we used provide a nuanced view of the patterns and contexts in which crimes occur, enabling a more targeted approach to prevention. By analyzing perpetrator and victim demographics, along with the specific characteristics of crime locations, researchers can identify underlying trends and potential hotspots of criminal activity. Including the borough in which the crime takes place adds another layer of geographic specificity, which is crucial for understanding regional variations in crime rates and the effectiveness of local law enforcement strategies. This deep-dive analysis helps in developing tailored interventions that are more likely to be effective in specific communities, ultimately aiding in the reduction of crime and enhancement of public safety.
...
ggplot(loc_counts, aes(x = reorder(LOC_CLASSFCTN_DESC, n), y = n, fill = LOC_CLASSFCTN_DESC)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = 'Crime Location Classification Distribution', x = 'Crime Location Classification', y = 'Count') +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

## Section 1.4: Offense Descriptions
