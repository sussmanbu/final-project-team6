...
The variables that intrigue us the most include:
...
**In our quest for deeper insights, we seek answers to the following questions:**
...
The data indicates that individuals identified as Black constitute the largest group of perpetrators, followed by a significantly smaller number of White Hispanics, and even fewer Whites and Black Hispanics. Asian/Pacific Islanders have the smallest count among the identified races. It could be inferred that understanding the racial distribution of perpetrators can be an important factor when analyzing crime data by borough, especially considering the demographic composition of each borough. The importance of analyzing perpetrator race lies in its potential to highlight areas where targeted social and law enforcement interventions may be needed.
...
It is noticeable that Black individuals constitute the largest group among the recorded victim races, with a notably high count. White Hispanics and Whites follow, with the former having a slightly higher count than the latter. The count for Asian/Pacific Islander and Black Hispanic victims is lower, and there is also a category for "Unknown," indicating instances where the victim's race was not recorded or identified. The distribution of victim race can be a crucial factor in understanding and predicting crime in different boroughs (BOROs) of New York City, as it might reflect underlying patterns of victimization related to social, economic, and possibly racial dynamics within these areas.
...
ggplot(vic_race, aes(x = reorder(VIC_RACE, n), y = n, fill = VIC_RACE)) + 
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Victim Race Distribution", x = "Victim Race", y = "Count", fill = "Victim Race") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5))
```
...
These plots explore the average salary distribution across the five boroughs of New York City (NYC). The first plot illustrates the average salary by work location borough, providing a comparative view of salary levels among the Bronx, Brooklyn, Manhattan, Queens, and other areas. Using a bar chart, the plot visualizes the mean salary for each borough, allowing for easy comparison of salary magnitudes. The second plot extends this analysis spatially by mapping the average salary by NYC borough onto a geographic representation of the city. Utilizing a map, the plot highlights spatial variations in average salary levels across different boroughs, with color intensity indicating salary magnitudes. This spatial perspective offers valuable insights into geographic disparities in salary distribution within NYC, providing a comprehensive understanding of economic dynamics across its diverse neighborhoods.

It is easy to visualize the disparity of income levels in NYC with these plots. We can see the vast disparity between the Borough Brooklyn and Manhattan.
...
ggplot(data = murder_rate, aes(x = BORO, y = murder_rate)) +
...
We want to make a model to see if our thesis and model correlate with eachother. We are predicting the Borough in which a crime takes place based on several predictors which we state below: 

For our predictors, we have first converted our characters into factors as statistical models require categorical variables encoded as factors. 

We choose the variables: Perpetrator Sex, Perpetrator Race, Location, Perpetrator age group, Victim Sex, Victim Age group, Victim Race, Offense Description Murder Flag
...
The first model we thought to use was Multinomial Logistic Regression, but this model only gave us an accuracy of **59%** which we were not satsifed with.
...
We then attempted to use a Random Forest model. We also only coose features that have atleast 200 to make sure the model has sufficient data to make a prediction.
...
The various lines here represent the various predictors and we observe that the error is going down proving to become a better model.

We got an accuracy of almost **84%** which is much better. However, due to the data having most of the offenses in a small subset of the boroughs we believe that the model is biased and does not have sufficient data for the other boroughs like Staten Island to predict it. It is also impossible to predict exactly because of the many variables that need to be considered for completely accurate predictions.

An accuray of 84% is still good however to show that these variables do in fact impact which Borough the crime takes place in and these Boroughs have income levels correlated with them as we can see in Section 1.4 above
